...
Robot::Robot(double s,double x,double y,double theta,double r,double a, int ID):
    velocity(s),coordinate_x(x),coordinate_y(y),theta(theta),robotsize_radius(r),alert(a), ID(ID){}

Robot::~Robot(){
    count_robots--;
}

double Robot::getVelocity() const{return velocity;}

void Robot::change_velocity(int s){velocity=s;}
